...
def get_foward_loss(pop1_dict, pop2_dict):# pop_dict={dB, init_x,init_c, GlobalParams, main_models}
  ...
  if trick=='no':
    ...
    for j in range(1, NT+1):
        ...
        x1 =x1+ (h1-(1/zeta1+1/gamma1)*yx1+1/gamma1*S+c1)*dt+sigma1*dB1[:,j].view(-1,1)
        x2 =x2+ (h2-(1/zeta1+1/gamma2)*yx2+1/gamma2*S+c2)*dt+sigma2*dB2[:,j].view(-1,1)
        
        c1=c1-1/beta1*yc1*dt
        c2=c2-1/beta2*yc2*dt
        
        yc1=yc1-yx1*dt+zc1*dB1[:,j].view(-1,1)
        yx1=yx1+zx1*dB1[:,j].view(-1,1)
        yc2=yc2-yx2*dt+zc2*dB2[:,j].view(-1,1)
        yx2=yx2+zx2*dB2[:,j].view(-1,1)
  ...
  if trick=='logit': ## use yx_tilde=logit(-yx), yx=-sigmoid(yx_tilde), dyx=-zx*(1+yx)*yx*dB
    ...
    for j in range(1, NT+1):
        ...
        yc1=yc1-yx1*dt+zc1*dB1[:,j].view(-1,1)
        yx1_tilde=yx1_tilde-(zx1**2)*(yx1+1/2)*dt+zx1*dB1[:,j].view(-1,1)
        yx1=-torch.sigmoid(yx1_tilde).view(-1,1)
        yc2=yc2-yx2*dt+zc2*dB2[:,j].view(-1,1)
        yx2_tilde=yx2_tilde-(zx2**2)*(yx2+1/2)*dt+zx2*dB2[:,j].view(-1,1)
        yx2=-torch.sigmoid(yx2_tilde).view(-1,1)

        S=(pi1*yx1.mean()/gamma1+pi2*yx2.mean()/gamma2)/(pi1/gamma1+pi2/gamma2)
        
    loss_yx1_tilde=nn.BCEWithLogitsLoss()(yx1_tilde,-target(x1,GlobalParams=pop1_dict['GlobalParams'],target_type='indicator'))
    loss_yc1=torch.mean((yc1)**2).to(device)
    loss_yx2_tilde=nn.BCEWithLogitsLoss()(yx2_tilde,-target(x2,GlobalParams=pop2_dict['GlobalParams'],target_type='indicator'))
    loss_yc2=torch.mean((yc2)**2).to(device)
    loss=(loss_yc1*1000+loss_yx1_tilde)+(loss_yc2*1000+loss_yx2_tilde)
    return loss.to(device)
  ...
  if trick=='clamp': ## use dyx=-zx*(1+yx)*yx*dB ONLY
    ...
    for j in range(1, NT+1):
        ...
        yc1=yc1-yx1*dt+zc1*dB1[:,j].view(-1,1)
        yx1=(yx1-zx1*(1+yx1)*yx1*dB1[:,j].view(-1,1)).clamp(min=-1,max=0)
        # yx1=(yx1-zx1*dB1[:,j].view(-1,1)).clamp(min=-1,max=0)
        yc2=yc2-yx2*dt+zc2*dB2[:,j].view(-1,1)
        yx2=(yx2-zx2*(1+yx2)*yx2*dB2[:,j].view(-1,1)).clamp(min=-1,max=0)
        # yx2=(yx2-zx2*dB2[:,j].view(-1,1)).clamp(min=-1,max=0)
    ...
    loss_yx2=torch.mean((yx2-target(x2,GlobalParams=pop2_dict['GlobalParams']))**2).to(device)
    loss_yc2=torch.mean((yc2)**2).to(device)
    loss=(loss_yc1*500+loss_yx1)+(loss_yc2*500+loss_yx2)
    return loss.to(device)

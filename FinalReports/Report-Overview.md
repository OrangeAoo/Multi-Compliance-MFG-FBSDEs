...
<img src="https://latex.codecogs.com/svg.image?$$\begin{alignat}{2}&\begin{cases}dX&space;_&space;t^{i}=(h^{k}&plus;g&space;_&space;t^{i}&plus;\Gamma&space;_&space;t^{i}&plus;C&space;_&space;t^{i})dt&plus;\sigma^{k}dB&space;_&space;t^{k},&X&space;_0^{i}\sim\mathcal{N}(v^k,\eta^k)\\dC&space;_&space;t^{i}=a&space;_&space;t^{i}dt,&C&space;_&space;0^{i}=0\\dY&space;_&space;t^{i}=Z&space;_&space;t^{k}dB&space;_&space;t^{k},&Y&space;_{T}^{i}=w\mathbf{1}_{X&space;_{T}^i<K},\\\end{cases}\\\textit{where:}&\\&Y&space;_&space;t^i=\mathbb{E}\left[w\mathbf{1}_{X&space;_{T}^i<K}|\mathcal{F}_&space;t\right]=w\mathbb{P}\left(X&space;_{T}^i<K\,|\,\mathcal{F}_&space;t\right)\\&S&space;_&space;t=\frac{\sum\limits&space;_{k\in\mathcal{K}}{\left(\frac{\pi^k}{\gamma^k}\mathbb{E}\left[Y&space;_&space;t^i\,|\,i\in\mathfrak{N}^k;\mathcal{F}_&space;t\right]\right)}}{\sum\limits&space;_{k\in\mathcal{K}}{\left(\frac{\pi^k}{\gamma^k}\right)}}\\&g&space;_&space;t^k=\frac{Y&space;_&space;t^k}{\zeta^k}\\&\Gamma&space;_&space;t^{k}=\frac{Y&space;_&space;t^{k}-S&space;_&space;t}{\gamma^{k}}\end{alignat}$$" title="$$\begin{alignat}{2}&\begin{cases}dX _ t^{i}=(h^{k}+g _ t^{i}+\Gamma _ t^{i}+C _ t^{i})dt+\sigma^{k}dB _ t^{k},&X _0^{i}\sim\mathcal{N}(v^k,\eta^k)\\dC _ t^{i}=a _ t^{i}dt,&C _ 0^{i}=0\\dY _ t^{i}=Z _ t^{k}dB _ t^{k},&Y _{T}^{i}=w\mathbf{1}_{X _{T}^i<K},\\\end{cases}\\\textit{where:}&\\&Y _ t^i=\mathbb{E}\left[w\mathbf{1}_{X _{T}^i<K}|\mathcal{F}_ t\right]=w\mathbb{P}\left(X _{T}^i<K\,|\,\mathcal{F}_ t\right)\\&S _ t=\frac{\sum\limits _{k\in\mathcal{K}}{\left(\frac{\pi^k}{\gamma^k}\mathbb{E}\left[Y _ t^i\,|\,i\in\mathfrak{N}^k;\mathcal{F}_ t\right]\right)}}{\sum\limits _{k\in\mathcal{K}}{\left(\frac{\pi^k}{\gamma^k}\right)}}\\&g _ t^k=\frac{Y _ t^k}{\zeta^k}\\&\Gamma _ t^{k}=\frac{Y _ t^{k}-S _ t}{\gamma^{k}}\end{alignat}$$" />

<img src="https://latex.codecogs.com/svg.image?$$\begin{cases}dX&space;_&space;t^{i}=(h^{k}&plus;g&space;_&space;t^{i}&plus;\Gamma&space;_&space;t^{i}&plus;C&space;_&space;t^{i})dt&plus;\sigma^{k}dB&space;_&space;t^{k},&X&space;_0^{i}\sim\mathcal{N}(v^k,\eta^k)\\dC&space;_&space;t^{i}=a&space;_&space;t^{i}dt,&C&space;_&space;0^{i}=0\\dY&space;_&space;t^{i}=Z&space;_&space;t^{k}dB&space;_&space;t^{k},&Y&space;_{T}^{i}=w\mathbf{1}_{X&space;_{T}^i<K},\\\end{cases}\\$$" title="$$\begin{cases}dX _ t^{i}=(h^{k}+g _ t^{i}+\Gamma _ t^{i}+C _ t^{i})dt+\sigma^{k}dB _ t^{k},&X _0^{i}\sim\mathcal{N}(v^k,\eta^k)\\dC _ t^{i}=a _ t^{i}dt,&C _ 0^{i}=0\\dY _ t^{i}=Z _ t^{k}dB _ t^{k},&Y _{T}^{i}=w\mathbf{1}_{X _{T}^i<K},\\\end{cases}\\$$" />

where:

<img src="https://latex.codecogs.com/svg.image?$$\begin{aligned}\textit{where:}&\quad\\&Y&space;_&space;t^i=\mathbb{E}\left[w\mathbf{1}_{X_{T}^i<K}|\mathcal{F}_t\right]=w\mathbb{P}\left(X_{T}^i<K\,|\,\mathcal{F}_t\right)\\&S&space;_&space;t=\frac{\sum\limits&space;_{k\in\mathcal{K}}{\left(\frac{\pi^k}{\gamma^k}\mathbb{E}\left[Y&space;_&space;t^i\,|\,i\in\mathfrak{N}^k;\mathcal{F}_&space;t\right]\right)}}{\sum\limits&space;_{k\in\mathcal{K}}{\left(\frac{\pi^k}{\gamma^k}\right)}}\\&g&space;_&space;t^k=\frac{Y&space;_&space;t^k}{\zeta^k}\\&\Gamma&space;_t^{k}=\frac{Y&space;_&space;t^{k}-S&space;_&space;t}{\gamma^{k}}\end{aligned}$$" title="$$\begin{aligned}\textit{where:}&\quad\\&Y _ t^i=\mathbb{E}\left[w\mathbf{1}_{X_{T}^i<K}|\mathcal{F}_t\right]=w\mathbb{P}\left(X_{T}^i<K\,|\,\mathcal{F}_t\right)\\&S _ t=\frac{\sum\limits _{k\in\mathcal{K}}{\left(\frac{\pi^k}{\gamma^k}\mathbb{E}\left[Y _ t^i\,|\,i\in\mathfrak{N}^k;\mathcal{F}_ t\right]\right)}}{\sum\limits _{k\in\mathcal{K}}{\left(\frac{\pi^k}{\gamma^k}\right)}}\\&g _ t^k=\frac{Y _ t^k}{\zeta^k}\\&\Gamma _t^{k}=\frac{Y _ t^{k}-S _ t}{\gamma^{k}}\end{aligned}$$" />

...
%% ------------------ NewCommand ------------------ %%
\newcommand{\MyGitHub}{\href{https://github.com/OrangeAoo/Multi-Period-Compliance-MFG-FBSDEs}{my GitHub repository}}
...
\newcommand{\MyEmail}{\href{mailto: ya2538@columbia.edu}{email}}
...
\newcommand{\ProfSC}{\href{https://www.stevenacampbell.com/}{Prof. Steven Campbell}\xspace}
% ------------------------ Main Doc ------------------------ %
...
\vspace{60pt}
\begin{center}
\bfseries\Huge\selectfont
Multi-Period Compliance MFG \\
with \\
Deep FBSDE Solver\\
\vspace{30pt}
\normalfont\LARGE
...
{\large\itshape\selectfont
This is an overview of the 2024 summer research supervised by \ProfSC, giving big pictures about:
\begin{enumerate}
    \item The problem aimed to solve.
    \item The algorithm and tricks implemented. 
    \item The example results and interpretations.
    \item The conclusion and takeaways.
\end{enumerate}
\vspace{30pt}

Visit \MyGitHub\ for more math, algorithm details, and code instructions\footnote{In case the hyperlink doesn't work, please visit \MyGitHub\ at: https://github.com/OrangeAoo/Multi-Period-Compliance-MFG-FBSDEs. Feel free to start an issue on \MyGitHub\ or directly reach out to me through \MyEmail\ (ya2538@columbia.edu) if you find any problems.}.
}
\vfill
...
\normalsize This work aims to extend the single-period compliance model in \cite{SC} to multi-period, proposing several tricks to improve the numeric stability of the Deep Solver for FBSDEs with jumps. After validating the model framework by reproducing the results in \cite{SC}, we consider an additional period, to which any remaining inventories after the first compliance period can be carried forward. For benchmarking, we run the single-period model for the 2 periods separately. Also, we compare different preferences of agents across sub-populations. This can reveal practical implications when making production strategies for companies in REC markets. 
...
\begin{minipage}[ht]{0.75\textwidth}
    ...
    \captionof{figure}{Market Prices: Joint (L) Vs Separate (R) Optimization}
    \label{fig:price}
\end{minipage}
\end{center}
...
A closer look at each sub-population reveals differences due to distinct ``preferences" and initial conditions. Regardless of perspective (long/short-term), the higher initial inventory of ``pop1" (green) makes them less motivated to work extra hours in the first period, despite a hidden drawback in baseline capacity. As a result, in the second compliance period, they often struggle to meet the quota and must purchase from ``pop2" (red), as shown by the red uptrends and green downtrends in Figures \ref{fig:decomp-gen-jnt}\ and \ref{fig:decomp-gen-sep}\ (where positive values indicate buying and vice versa).

\paragraph{Model Performances}

As is seen from Figure \ref{fig:FwdLoss}\ and \ref{fig:terminal-values-jnt}\ ,  the algorithms converge quickly after 200 epochs, and their learned terminal conditions align closely with target values (displayed as black dots).
...
\begin{minipage}[ht]{0.4\textwidth}
    ...
    \captionof{figure}{Forward Loss}
    \label{fig:FwdLoss}
\end{minipage}
\end{center}
\vspace{-7pt}
\begin{center}
\begin{minipage}[ht]{0.82\textwidth}
    % \centering
...
In this study, we successfully extended the single-period compliance model to a 2-period setting, providing a motivating framework for modeling REC markets using coupled FBSDEs. 

\textbf{Methodologically}, the additional smoothing and scaling novel numeric techniques including \textit{sigmoid approximation}, \textit{numeric clamp}, and \textit{logit-clamp transformation}, contributed to improved model stability and convergence. 

\textbf{Practically}, the results highlighted key differences between agent preferences across sub-populations. Additionally, by benchmarking joint optimization to a separate one, we revealed significant insights into agent behaviors and market prices influenced by long-term versus myopia perspectives on compliance strategies, giving practical and instructive takeaways not only to the REC markets but also to one's life.
\textit{\begin{itemize}
...
This work sets a foundation for further exploration into higher-dimensional MFG FBSDEs and more realistic REC scenarios. \textbf{In the next phase, one is encouraged to add more compliance periods and/or inventory carryforward (or expiration) rules.} And to facilitate the experiments, the models are structured as user-friendly packages, documented by comprehensive instructions, and made public in \MyGitHub, which is open to suggestions and/or contributions.
...
\begin{minipage}[ht]{0.75\textwidth}
    \centering
    \includegraphics[]{FinalReports/Illustration_diagrams/SigmoidApprox.png}
    \captionof{figure}{Sigmoid Approximation}

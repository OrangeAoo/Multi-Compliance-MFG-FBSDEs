...
%% ------------------ NewCommand ------------------ %%
\newcommand{\MyGitHub}{\href{https://github.com/OrangeAoo/Multi-Compliance-MFG-FBSDEs.git}{my GitHub repository}}
...
This is an overview of the 2024 summer research supervised by Prof. Steven Campbell, giving big pictures about:
\begin{enumerate}
...
Visit \MyGitHub\ for more math, algorithm details, and code instructions\footnote{Please start an issue on GitHub or reach out to me through \MyEmail\ if you find any problems.}.
}
\vfill
...
% Starting at generally lower initial level (\(v^1>v^2\))
...
convergence and stability, which are open for experiments.
...
The results highlighted key differences between agent preferences across sub-populations. Additionally, by benchmarking joint optimization to a separate one, we revealed significant insights into agent behaviors and market prices influenced by long-term versus myopia perspectives on compliance strategies, giving practical and instructive \textbf{takeaways} not only to the REC markets but also to one's life.
\textit{\begin{itemize}
...
\textbf{Methodologically}, the novel numeric techniques including \textit{sigmoid approximation}, \textit{numeric clamping}, and \textit{logit transformation}, contributed to improved model stability and convergency. 

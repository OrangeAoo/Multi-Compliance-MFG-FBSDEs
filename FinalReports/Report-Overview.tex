...
Here we use the sample results by model with terminal target function: $0.25\sigma{\left(X_T^i\right)}$, where $T=T_1,\,T_2$.

The learnt controls are shown as the first row and the accumulative generations by different means as the second row, correspondingly. And the green color denotes the sub-population 1 while red the sub-population 2. (Same applies to all the later plots.)

\hspace{0.8\textheight}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{FinalReports/Illustration_diagrams/Seprt-2A2P-Sigmoid-ResExamples/Rates.png}\\
    \includegraphics[width=0.8\textwidth]{FinalReports/Illustration_diagrams/Seprt-2A2P-Sigmoid-ResExamples/AccumRates.png}\\
    \caption{Decomposed Generation}
    \label{fig:terminal-values}
\end{figure}

\newpage

And the plots below display a rather good convergence of learnt terminal values to their \textit{sigmoid targets}. 

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{FinalReports/Illustration_diagrams/Seprt-2A2P-Sigmoid-ResExamples/sigmoid_target.png}\\
    \caption{Decomposed Generation}
...
\begin{figure}[h]
    ...
    {FinalReports/Illustration_diagrams/Seprt-2A2P-Sigmoid-ResExamples/AccumRates.png}\\
    \caption{Decomposed Generation of Separate-Optimization}

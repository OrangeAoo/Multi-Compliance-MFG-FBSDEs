...
class plot_results():
    ...
    def Inventory_And_Price(self,Histogram=True): #,single_sample=False):
        ...
        if Histogram==True:
          ...
          sns.histplot(data=x2_t2, bins=100,stat='count',alpha=0.6,color= 'green')
          sns.kdeplot(x2_t2, color="green",label='P2')
          plt.xlabel("$X_{T_2}$")
          plt.ylabel("Count")
          plt.legend()

          plt.subplot(233)
          plt.title("Distribution of $X_{T_3}$")
          sns.histplot(data=x1_t3, bins=100,stat='count',alpha=0.6,color= 'green')
          sns.kdeplot(x1_t3, color="green",label='P1')
          sns.histplot(data=x2_t3, bins=100,stat='count',alpha=0.6,color= 'green')
          sns.kdeplot(x2_t3, color="green",label='P2')
          plt.xlabel("$X_{T_3}$")
          plt.ylabel("Count")
          plt.legend()
          # if self.savefigs and self.to_path:
          ...
          # ## -------------------------------- Population 2 -------------------------------- ##
          # plt.suptitle("Inventory @ Delivery - $T_2$")
          # plt.subplot(234)

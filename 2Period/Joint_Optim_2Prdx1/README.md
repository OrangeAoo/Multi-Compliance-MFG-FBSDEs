...
- ##  `Params`
    ...
    - __Parameters:__ 
        ...
        > :warning: __NOTE:__ there are only 4 sets valid combiantions of `target_type`, `trick`, and `loss_type` 
...
---
- ## `utils` (Functions)
    - > __Sample_Init__ ( _GlobalParams_)
    ...
    - > __SampleBMIncr__ ( _GlobalParams_)
        ...
        - __Returns:__
            ...
            > :warning: __NOTE:__ the $(dB)_{t\ge 0}$ matrix should vary from sub-population to sub-population, yet remain the same for agents within the same sub-population.  
...
- ## `plot_results`
    ...
    - __Attributes:__
        ...
        - __pop2\_plot__ (_dict_) - a dictionary of pop2 paths (_array_) for plotting. Each _array_ within the dictionary includes only `number_of_paths` out of `NumTrain` selected samples.
            > :warning: __NOTE:__ the _arrays_ in `pop1_plot` and `pop2_plot` are obtained by detaching the _tensors_ in `pop1_path_dict` and `pop2_path_dict`.
    ...
    - > __FwdLoss__ ( _log=True_)
        Plots the forward losses against training epochs.
